...
function countdown(duration, api, event) {
  const isMinutes = duration.includes("m");
  const countDownInterval = setInterval(() => {
    let remainingTime = isMinutes ? duration * 60 : duration;
    remainingTime--;
    if (remainingTime <= 0) {
      clearInterval(countDownInterval);
      api.sendMessage("Time's up!", event.threadID);
    } else {
      const minutes = Math.floor(remainingTime / 60);
      const seconds = remainingTime % 60;
      api.sendMessage(`Countdown: ${minutes}:${seconds < 10 ? '0' : ''}${seconds}`, event.threadID);
    }
  }, 1000);
}
const actions = {
  ...
  "$countdown": (api, event, duration) => {
    countdown(duration, api, event);
  },
};
api.listen((err, event) => {
  try {
    ...
    const actions = {
      ...
      "$stop": stopListenerFunc,
      "$countdown": (api, event) => {
        const duration = event.body.split(" ")[1];
        countdown(duration, api, event);
      },
    };
